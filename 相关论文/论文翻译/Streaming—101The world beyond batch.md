...
举例来说，跟踪每秒发送到全球规模Web服务的请求数， 来检测中断，计算这些请求的速率是处理时间窗口的完美用法。

处理时间窗口有一个很大的缺点：如果数据中带有事件时间，同时数据必须按照事件时间顺序处理，因为处理时间和事件时间之间有可变的偏差，所以使用处理时间处理无法应对这种情况。

例1，一个移动App收集使用统计信息以供后续处理。如果给定的移动设备离线（短时间内连接失败，调为飞行模式等），则在该设备上线之前不会上传数据。这意味着数据可能会遇到事件时间偏差的分钟，小时，天，周或更长时间。使用处理时间窗口时，从这样的数据集中无法推断出设备离线或者其他有用的信息。

例2，当整个系统正常时，一些分布式输入源可能似乎能够提供事件时间有序（或非常接近）的数据。但是输入源的事件时间偏差，即便是在正常状态下也并不意味着会始终如一。在全球性的场景中，Web服务跨越了多个大陆，收集数据受限于跨洋线路的带宽，进一步降低了带宽和/或提高了延迟，那么输入数据的一部分突然可能开始以比以前更大的偏差到达系统。如果通过处理时间窗口处理数据，则窗口不再代表其中实际发生的数据;相反，窗口代表事件到达系统的时间窗口。

在这两种情况下，要想事件按照事件时间顺序的处理，需要的是事件时间窗口。

##### 基于事件时间窗口

事件时间窗口，按照事件的实际发生时间，将数据切分，放到合适的窗口去，每个窗口是一个有限数据集，下图显示了将无限数据源中的数据，按照1小时长度的固定窗口进行切分的示例：

![](../img/eventtime_fixed_windows.jpg)

​																**基于事件时间划分固定窗口**

该图中的实线白线标出了两个感兴趣的两类数据。从图中可以看出，这两类数据都到达处理时间窗口，但是与它们所属的事件时间窗口不匹配。 因此，想按照事件时间进行处理，如果使用处理时间窗口，则计算结果将不正确。需要按照时间时间进行处理需要使用事件时间窗口。

在处理无限数据的时候，使用事件时间窗口，可以创建动态大小的窗口。例如会话窗口，此种情况下，使用固定窗口会将紧密相关联数据分隔到不同的窗口(之前在“无限数据”的会话窗口示例中说明过)：

![](../img/eventtime_session_windows.jpg)

​																**基于事件时间划分会话窗口**

功能强大是有代价的，事件时间窗口也不例外。事件时间窗口有两个显着的缺点，因为窗口通常比窗口本身的实际长度(处理时间)更长，对于处理时间，机器时间超过窗口末尾，将数据发送给下游，窗口就可以销毁了，而事件时间窗口由于延迟的存在，机器时间超过窗口末尾，此时还不能确定事件是否全部到达（几乎可以肯定没有完全到达），需要再等待一段时间：

- **缓存**

由于窗口的生命周期变长，需要缓存更多的数据，持久化存储通常是最廉价缓存方式(其他主要是CPU，网络带宽和RAM)。

- **完整性**

由于难以知道一个窗口是否所有的数据都到齐了，所以我们根本就没有办法精准的确定在哪个时刻进行计算给出结果。但是对于许多类型的输入源，系统可以通过像MillWheel的Watermark(第二部分将详细讨论)来给出窗口的数据到齐时刻的相当准确的启发式估计。但是在必须要保证决定正确的情况下(例如计费)，唯一的选择是为Pipeline提供一种方式来表达何时计算结果，以及这些结果如何随着时间的推移而更正。处理窗口的完整性(或缺乏)是一个复杂且有趣话题，在第2部分中将结合具体的例子进行阐述。